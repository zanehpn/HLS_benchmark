...
class SystemDesign(BaseModel):
    """systen definition"""
    name: str = Field(description="Name of the system application.")
    ...
    software_host: List[str] = Field(description="Host Code Responsibilities")
    hardware_kernel: List[str] = Field(description="FPGA PL Kernels Responsibilities")
    communication: List[str] = Field(description="Outline the Data Flow and Communication Between Host and FPGA")
...
class HardwareCompEvaluator(BaseModel):
    """system design evaluation"""
    fail: str = Field(description="true If the hardware design dose not have the same function as the software presudo or steps.")

class HardwareEvaluator(BaseModel):
    """system design evaluation"""
...
class ModuleEvaluator(BaseModel):
    ...
    placeholders: str = Field(description="NA if the code do not have any placeholders or otherwise missing components from a complete synthesizable code in this module, otherwise explain.")
...
class SoftwareResponse(BaseModel):
    """software design to the user"""
    name: str = Field(description="host.cpp")
    description: str = Field(description="Brief module description.")
    host_code: str = Field(description="Complete host code written in C++ using Xilinx Vitis Runtime (XRT) APIs without any placeholders.")

### Module Design agent
class CodeModuleResponse(BaseModel):
    """hardware module to the user"""
    name: str = Field(description="Name of the module.")
    description: str = Field(description="Brief module description.")
...
class ConfigGenerator(BaseModel):
    ...
    name: str = Field(description="Name of the configuration generator.")
    description: str = Field(description="Vitis configuration file (.cfg) for an FPGA platform with the following specifications:")
    cfg_file: str = Field(description="Complete Vitis configuration file including platform, kernel_frequency, kernel instance, kernel mapping and so on without any placeholders.")
...
class GenericToolCallingAgent(object):
    ...
    def __init__(self, model: ChatOpenAI,
                 prompt: BaseChatPromptTemplate=system_agent_prompt,
...
class Engineer(object):
    ...
    def __init__(self, model: ChatOpenAI, evaluator_model: ChatOpenAI, retriever: Any, solution_num: int = 0):
        ...
        self.system_solution_result = None
        self.software_design_result = None
        self.hardware_solution_result = None
    ...
    #### system design agent
        self.system_design_agent = GenericToolCallingAgent(
            model=model, prompt=system_agent_prompt,tools=[search_web], response_format=SystemDesign)

    #### hardware Design Agent
        self.hardware_design_agent = GenericToolCallingAgent(
            model=model, prompt=hardware_design_agent_prompt, response_format=HardwareDesign)

    #### hardware_design_transformation_compare_prompt
        self.hardware_design_transformation_compare_agent = GenericToolCallingAgent(
            response_format = HardwareCompEvaluator,
            prompt = hardware_design_transformation_compare_prompt,
            model = evaluator_model,
            tools = [search_web, python_run]
        )
    #### hardware Design Evaluator Agent
        self.hardware_design_evaluator_agent = GenericToolCallingAgent(
            response_format=HardwareEvaluator,
            prompt=hardware_design_agent_evaluation_prompt,
            model=evaluator_model, #model,
            tools=[]
            )
    #### software Design Agent
        self.software_design_agent = GenericToolCallingAgent(
            model=model, prompt=software_agent_prompt,
            tools=[search_web], response_format=SoftwareResponse
            )

    #### hardware Redesign Agent
        self.hardware_redesign_agent = GenericToolCallingAgent(
            prompt=hardware_design_agent_update_prompt,
            response_format=HardwareDesign,
            model=model,
            tools=[]
            )

    #### Module Design Agent
        self.hardware_module_design_agent = GenericToolCallingAgent(
            model=model, prompt=hardware_module_design_agent_prompt,
    ...
    #### Module Evaluator agent
        ...
        self.config_generator_agent = GenericToolCallingAgent(
            model=model, prompt=config_generator_agent_prompt,
            response_format=ConfigGenerator
            )
        
        if "3.5" in model.model_name:
            self.hardware_module_design_agent._max_failurecount = 3
            self.hardware_design_agent._max_failurecount = 3
        ...
        self.workflow.add_node("system_solution", self.system_solution)
        self.workflow.add_node("hardware_solution", self.hardware_solution)
        self.workflow.add_node("software_design", self.software_design)
        self.workflow.add_node("hardware_redesign_solution", self.hardware_redesign_solution)
        self.workflow.add_node("hardware_compare_evaluation", self.hardware_compare_evaluation)
        self.workflow.add_node("hardware_evaluation", self.hardware_evaluation)
        self.workflow.add_node("hardware_modular_design", self.hardware_modular_design) 
        self.workflow.add_node("hardware_modular_integrator", self.hardware_modular_integrator)
        self.workflow.add_node("module_evaluator", self.module_evaluator)
        self.workflow.add_node("config_generator", self.config_generator) 
        ...
        #         "hardware_solution": "hardware_solution",
        #      },
        #     )

        self.workflow.add_edge("system_solution", "software_design") 
        self.workflow.add_edge("software_design", "hardware_solution")
        self.workflow.add_edge("hardware_solution", "hardware_compare_evaluation")
        self.workflow.add_conditional_edges(
            "hardware_compare_evaluation",
            (lambda x: x['keys']['goto']),
            {
                "redesign":"hardware_redesign_solution",
        ...
        self.workflow.add_conditional_edges(
            ...
            {
                "redesign":"hardware_redesign_solution",
                "modular": "hardware_modular_design",
            },
        )
        self.workflow.add_edge("hardware_redesign_solution", "hardware_evaluation")
        self.workflow.add_edge("hardware_modular_design", "hardware_modular_integrator")

        #self.workflow.add_edge("software_design", "module_evaluator")
        self.workflow.add_edge("hardware_modular_integrator", "module_evaluator")
        self.workflow.add_conditional_edges(
            "module_evaluator",
            (lambda x: x['keys']['goto']),
            {
                "modular":"hardware_modular_integrator",
                "end": "config_generator",
            },
        )
        self.workflow.add_edge("config_generator", END)
        # Compile
        self.app = self.workflow.compile()
        graph = self.app
        try:
    ...
    def system_solution(self, state):
        print("---system design---")
        system_solution_human = HumanMessage(f"""Transform the input context provided by the user into CPU-FPGA system design according to the following goals and requirements. \
        The system can be divided into two parts including host code and FPGA kernel code.
        The language of choice for coding host code is C++.                                     
        The language of choice for coding FGGA kernel code is Xilinx HLS C++.
        ...
        #try:                         
        self.system_solution_result = self.system_design_agent.invoke(
            {
                "messages" : [system_solution_human],
                }
            )
        result = HumanMessage(str(self.system_solution_result), name="system_designer")
        print(result)
        ...
        with open(f'solution_{self.solution_num}/solution.txt', 'w+') as file:
            file.write(str(self.system_solution_result))

        return{
            "messages": [result],
            "sender": "system_designer"
        }

    def hardware_solution(self, state):
        print("---hardware design---")
        #messages = "\n".join([str(item) if item.name in ['compute', 'search'] else '' for item in state["messages"]])
        hardware_solution_human = HumanMessage(f"""Design the architecture graph for the following goals, requirements and input context provided by the user. \
        The language of choice for coding the design is Xilinx HLS C++. It is the hardware part of system design. 
        It will collaborate with software design (the host code) to accelerate the system.
        The name of top module of hardware must be the same as the FPA kernel name of host code.
        To help you further, you are also provided with literature review performed by another agent.
        ...
        {self.system_solution_result} 

        software design results:
        {self.software_design_result}       
        ...
        #try:                         
        self.hardware_solution_result = self.hardware_design_agent.invoke(
            {
                "messages" : [hardware_solution_human],
                }
            )
        self.hardware_solution_result.graph.sort(key=lambda x: len(x.connections)) # sort by number of outward connections
        result = HumanMessage(str(self.hardware_solution_result), name="designer")
        print("hardware_solution_result:", self.hardware_solution_result)
        if not os.path.exists(f'solution_{self.solution_num}/pl'):
            os.makedirs(f'solution_{self.solution_num}/pl')
        with open(f'solution_{self.solution_num}/solution.txt', 'w+') as file:
            file.write(str(self.hardware_solution_result))

        return {
            "messages": [result],
            "sender": "designer",
            "keys": {
            "goto": "hardware_evaluation",  # Explicitly set the next node
            "hardware_solution_result": self.hardware_solution_result,  # Pass result
            },
        }

    def hardware_compare_evaluation(self, state):
        print("---hardware compare Evaluation---")
        self.hardware_solution_result.graph.sort(key=lambda x: len(x.connections)) # sort by number of outward connections
        hardware_design = self.hardware_solution_result.graph
        hardware_compare_evaluation_human = HumanMessagePromptTemplate.from_template(
            """  
        ...
        self.hardware_evaluation_result = self.hardware_design_transformation_compare_agent.invoke(
            {
                "messages" : hardware_compare_evaluation_human.format_messages(
                        goals=self.goals,
                        input_context=self.input_context,
                        requirements=self.requirements,
        ...
        result = HumanMessage(str(self.hardware_evaluation_result), name="evaluator")

        return{
            "messages": [result],
            "sender": "evaluator",
            "keys": {
                "goto": "redesign" if self.hardware_evaluation_result.fail else "hardware_evaluation",
                "eval_results": self.hardware_evaluation_result,
                },
        }

    
    def hardware_evaluation(self, state):
        print("---hardware Design Evaluation---")
        self.hardware_solution_result.graph.sort(key=lambda x: len(x.connections)) # sort by number of outward connections
        hardware_design = self.hardware_solution_result.graph
        hardware_evaluation_human = HumanMessagePromptTemplate.from_template(
            """  
            You are provided with the overal design goals and requirements, a literature review, the overal system design and the desired coding language in the following.
            Your job is to assess the system design based on the given information. Be meticulous.
            The design is coded in Xilinx HLS C++.
        ...
        self.hardware_evaluation_result = self.hardware_design_evaluator_agent.invoke(
            {
                "messages" : hardware_evaluation_human.format_messages(
                        goals=self.goals,
                        requirements=self.requirements,
                        methodology=self.lit_review_results.methodology,
                        implementation=self.lit_review_results.implementation,
        ...
        #    file.write(str(self.hardware_evaluation_result))

        return{
            "messages": [result],
            "sender": "evaluator",
            "keys": {
                "goto": "redesign" if self.hardware_evaluation_result.fail else "modular",
                "eval_results": self.hardware_evaluation_result,
                },
        }
    
    def hardware_redesign_solution(self, state):
        ...
        {str(self.hardware_solution_result)}
        ...
        try:                         
            self.hardware_solution_result = self.hardware_redesign_agent.invoke(
                {
        ...
        self.hardware_solution_result.graph.sort(key=lambda x: len(x.connections)) # sort by number of outward connections
        result = HumanMessage(str(self.hardware_solution_result), name="redesigner")
        if not os.path.exists(f'solution_{self.solution_num}'):
            os.makedirs(f'solution_{self.solution_num}')
    ...
    def hardware_modular_design(self, state):
        print("---Modular design---")
        state_dict = state["keys"]
        hardware_design = self.hardware_solution_result.graph
        Modules = []
        print("Modular hardware_design:", hardware_design)
        for module in hardware_design:
            response = self.hardware_module_design_agent.invoke(
                {
                    "messages": hardware_modular_design_human_prompt.format_messages(
                        goals=self.goals,
                        requirements=self.requirements,
                        methodology=self.lit_review_results.methodology,
                        implementation=self.lit_review_results.implementation,
                        system_design=self.system_solution_result,
                        hardware_design=str(hardware_design),
            ...
            print("Modular design response:",response)
    ...
    def module_evaluator(self, state):
        ...
        state_dict = state["keys"]
        hardware_design = state_dict["coded_modules"]
...
You are provided with the overal design goals and requirements, a literature review, the overal system design, 
modules that are coded so far and the module that you will be coding.
...
"""
        ...
        response = self.module_evaluator_agent.invoke(
            {
                "messages": module_evaluator_prompt_human.format_messages(
                    ...
                    goals=self.goals,
                    requirements=self.requirements,
                    coded_modules='\n\n'.join([module.name + "\n" + module.module_code for module in hardware_design])
                    #software_design = self.software_design_result
                    )
                }
            )
        ...
        'keys' : {'goto': goto,
                  'feedback': response,
                  'coded_modules': hardware_design,
                  },
        }

    def hardware_modular_integrator(self, state):
...
"""
        ...
        for module in hardware_design:
            response = self.hardware_module_design_agent.invoke(
                {
                    "messages": module_agent_prompt_human.format_messages(
                        ...
                        methodology=self.lit_review_results.methodology,
                        implementation=self.lit_review_results.implementation,
                        hardware_design="\n".join([str(item) for item in hardware_design]),
                        modules_built=str(Modules), 
                        current_module=str(module)
                        )
                    }
            ...
            if response.header_file != "NA":
                with open(f"solution_{self.solution_num}/pl/{module.name}.h", "w") as f:
                    f.write(response.header_file)               
            with open(f"solution_{self.solution_num}/pl/{module.name}.{utils.find_extension(self.language)}", "w") as f:
                f.write(response.module_code)
    ...
    def software_design(self, state):
        print("---software design---")
        software_agent_prompt_human = HumanMessagePromptTemplate.from_template(
""" You should write the host code according to below rules
Goals:
{goals}
...
"""
        ...
        self.software_design_result = self.software_design_agent.invoke(
                {
                    "messages": software_agent_prompt_human.format_messages(
                        goals=self.goals,
                        requirements=self.requirements,
                        methodology=self.lit_review_results.methodology,
                        implementation=self.lit_review_results.implementation,
                        system_solution= self.system_solution_result
                        )
                    }
                )
        if not os.path.exists(f'solution_{self.solution_num}/host'):
            os.makedirs(f'solution_{self.solution_num}/host')
        with open(f"solution_{self.solution_num}/host/host.cpp", "w") as f:
            f.write(self.software_design_result.host_code) 
            print("write the code into host.cpp")           

        return{
            "messages": [self.software_design_result],
            "sender": "software_designer",
        }

    def config_generator(self, state):
        print("---config generation---")
        config_generator_agent_prompt_human = HumanMessagePromptTemplate.from_template(
""" You should write the host code according to below rules
Goals:
{goals}
...
"""
        ...
        self.config_generator_result = self.config_generator_agent.invoke(
                {
                    "messages": config_generator_agent_prompt_human.format_messages(
                        goals=self.goals,
                        requirements=self.requirements,
                        methodology=self.lit_review_results.methodology,
                        implementation=self.lit_review_results.implementation,
                        system_solution= self.system_solution_result,
                        hardware_solution= self.hardware_solution_result
                        )
                    }
                )
        
        with open(f"solution_{self.solution_num}/design.cfg", "w") as f:
            f.write(self.config_generator_result.cfg_file) 
            print("write the code into config file")           

        return{
            "messages": [self.config_generator_result],
            "sender": "config_designer",
        }
